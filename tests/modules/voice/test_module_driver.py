"""
Unit tests for `VoiceModuleDriver`, mirroring
`tests/modules/generation/test_module_driver.py`'s own wiring shape.
"""
...
from parika.modules.voice.module_driver import (
...
def test_exact_two_capabilities_registered(wiring, logger) -> None:
    assert len(_VOICE_TOOL_SPECS) == 2

    expected_ids = {"voice.speech_to_text", "voice.text_to_speech"}
    assert {spec.tool_capability_id for spec in _VOICE_TOOL_SPECS} == expected_ids


def test_provider_capabilities_use_distinct_speech_categories(logger) -> None:
    categories = {spec.provider_category for spec in _VOICE_TOOL_SPECS}
...
def test_disabled_module_registers_nothing(wiring, logger) -> None:
    capability_registry, tool_manager, brain = wiring
...
def test_default_operation_registry_is_created_when_not_supplied(wiring, logger) -> None:
    capability_registry, tool_manager, brain = wiring
    ...
    assert isinstance(driver.operation_registry, TtsOperationRegistry)


def test_shared_operation_registry_is_used_when_supplied(wiring, logger) -> None:
    capability_registry, tool_manager, brain = wiring
    ...
    assert driver.operation_registry is registry


def test_default_language_preference_is_created_when_not_supplied(wiring, logger) -> None:
    capability_registry, tool_manager, brain = wiring
    ...
    assert isinstance(driver.language_preference, VoiceLanguagePreferenceStore)


def test_shared_language_preference_is_used_when_supplied(wiring, logger) -> None:
    capability_registry, tool_manager, brain = wiring
    ...
    assert driver.language_preference is preference


def test_language_preference_is_seeded_from_voice_configuration(wiring, logger) -> None:
    capability_registry, tool_manager, brain = wiring

    class _HindiDefaultConfiguration:
        def get(self, key, default=None):
            if key == "voice.input_language":
                return "hi"
            if key == "voice.output_language":
                return "hi"
            return default
    ...
    driver = VoiceModuleDriver(
        ...
        brain=brain,
        logger=logger,
        configuration=_HindiDefaultConfiguration(),
    )
