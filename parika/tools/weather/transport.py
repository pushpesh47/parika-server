"""
PARIKA Weather Tool - HTTP Transport

Defines the HttpTransport contract used by the Weather Tool to issue
outbound HTTP requests, together with the default implementation
built on the Python standard library.

Mirrors `parika/tools/web_search/transport.py`'s pattern exactly
(isolating transport behind a small protocol so geocoding/forecast
logic can be exercised deterministically in tests without depending
on real network access) but is kept as this Tool's own, independent
module rather than an import from `web_search`, matching the
project's convention of each Tool package being self-contained.
"""
...
from .exceptions import WeatherNetworkError, WeatherTimeoutError

DEFAULT_USER_AGENT = "PARIKA-WeatherTool/1.0"
