"""
PARIKA Web Search Tool - Mojeek Search Backend (HTML)

A `SearchBackend` implementation backed by Mojeek's public HTML
search results page. Mojeek runs its own independent index (unlike
most other free search endpoints, which are thin layers over Bing or
Google) and applies comparatively light anti-bot restrictions, making
it a useful, no-API-key failover option. Requires no configuration
and is always considered available (see `provider_registry.py`).
...
MOJEEK_HTML_ENDPOINT = "https://www.mojeek.com/search?q={query}"


class _MojeekResultParser(HTMLParser):
    """
    Extracts search results from a Mojeek HTML results page.

    Mojeek's organic results are `<li class="result">` blocks
    containing a title anchor tagged `class="title"` and a snippet
    paragraph tagged `class="s"`.
    """
    ...
    def handle_starttag(
        ...
        if tag == "a" and "title" in classes and not self._title_captured:
...
class MojeekHtmlSearchBackend:
    """
    SearchBackend implementation using Mojeek's HTML results endpoint.
    ...
    def search(
        ...
        *,
        ...
        """
        Perform a web search using Mojeek's HTML endpoint.
        ...
        Raises:
            ...
            WebSearchNetworkError:
                ...
                Mojeek responds with an HTTP error status.
        """

        validate_search_arguments(query, max_results)

        url = MOJEEK_HTML_ENDPOINT.format(query=quote_plus(query))
        ...
        raise_if_http_error(response, provider_name="Mojeek")
